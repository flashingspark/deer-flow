...
## Local city statistics workflow

Local-only city statistics code lives in `skills/custom/china-city-statistics/scripts`. `models.py` owns the JSON contract and indicator catalog; `database.py` validates batches and writes a single SQLite transaction. Run its `test_database.py` via unittest with the backend venv; tests must use temporary databases. Preserve existing data and back up before changes. The local `city_statistics.py` CLI imports JSON files and queries SQLite using the Ubuntu backend Python. The city-statistics MCP server is disabled. Host bash remains disabled; native tools in backend/city_statistics_tools.py call authenticated loopback FastAPI city statistics CRUD without a shell.
