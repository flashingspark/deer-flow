...
## Local city statistics tools

`city_statistics_tools.py` exposes schema/save/query/update/delete through HTTP to `/api/city-statistics`. It uses the existing Gateway internal authentication and CSRF double-submit headers, with a fixed loopback destination. `city_statistics_store.py` loads the existing custom skill's models/storage without duplicating the schema. Routes offload SQLite operations to the thread pool. The existing shared database is retained (no owner columns/migration). Test with `python -m pytest tests/test_city_statistics_native.py -q`; use temporary DBs only.
