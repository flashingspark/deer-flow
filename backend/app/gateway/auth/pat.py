...
_PAT_ROUTE_RULES: tuple[tuple[frozenset[str], re.Pattern[str]], ...] = (
    # Agent API demo notes use the existing thread lifecycle scopes. They are
    # user-owned records, and the router performs the final owner check.
    (frozenset({"GET"}), re.compile(r"^/api/agent-notes$")),
    (frozenset({"GET", "PATCH", "DELETE"}), re.compile(r"^/api/agent-notes/[^/]+$")),
    (frozenset({"POST"}), re.compile(r"^/api/agent-notes$")),
