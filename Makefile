...
ifeq ($(OS),Windows_NT)
    ...
    # Use PowerShell directly for the interactive dev launcher. Calling a
    # .cmd wrapper makes cmd.exe show "Terminate batch job?" on Ctrl+C and
    # prevents the wrapper from normalizing the resulting exit code.
	RUN_DEV_SCRIPT = powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\run-dev-with-git-bash.ps1
else
    PYTHON ?= python3
...
dev:
	@$(PYTHON) ./scripts/check.py
	@$(RUN_DEV_SCRIPT) ./scripts/serve.sh --dev
