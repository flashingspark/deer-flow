...
# Some Windows uv installations can leave the user-level cache path in a
# state where uv reports ERROR_ALREADY_EXISTS while initializing it. Keep the
# local launcher self-contained by using an ignored, writable project cache
# unless the operator explicitly supplied UV_CACHE_DIR.
...
if ! $SKIP_INSTALL; then
    ...
    # pnpm 10 asks for confirmation before removing an incomplete
    # node_modules directory. The install also runs before the background
    # frontend process gets a TTY, so make this cleanup non-interactive.
    (cd frontend && "$DEERFLOW_PNPM_PYTHON" "$DEERFLOW_PNPM_RUNNER" install --silent --config.confirmModulesPurge=false) || { echo "✗ Frontend dependency install failed"; exit 1; }
