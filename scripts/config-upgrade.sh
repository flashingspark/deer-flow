...
# Keep config upgrades independent from a broken Windows user-level uv cache.
# serve.sh sets the same default later for the long-running services, but this
# script runs before that initialization during `make dev`.
