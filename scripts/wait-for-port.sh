...
is_port_listening() {
    # Windows netstat is available without the permissions required by
    # Get-NetTCPConnection on some hardened PowerShell installations. Its
    # columns are: protocol, local address, foreign address, state, PID.
    if command -v netstat.exe >/dev/null 2>&1; then
        if netstat.exe -ano -p TCP 2>/dev/null |
            awk -v port="$PORT" '$1 == "TCP" && $4 == "LISTENING" && $2 ~ (":" port "$") { found=1 } END { exit !found }'; then
            return 0
        fi
    fi
